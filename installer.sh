...
setup_bashrc_path() {
  ...
  # Remove old single-entry path export if present (idempotent upgrade)
  sed -i "\|export PATH=${SANDCASTLE_HOME}/docker-runtime/bin:\\\$PATH|d" "$profile" 2>/dev/null || true
  ...
  sed -i "\|export PATH=${SANDCASTLE_HOME}/docker-runtime/bin:\\\$PATH|d" "$bashrc" 2>/dev/null || true
...
write_admin_script() {
  cat > "${SANDCASTLE_HOME}/bin/sandcastle-admin" <<'ADMINEOF'
#!/bin/bash
...
cmd_install() {
  ...
  command -v zstd  &>/dev/null || pkgs_to_install+=(zstd)
  command -v rsync &>/dev/null || pkgs_to_install+=(rsync)

  if [ ${#pkgs_to_install[@]} -gt 0 ]; then
    info "Installing prerequisites: ${pkgs_to_install[*]}..."
    apt-get install -y "${pkgs_to_install[@]}" >/dev/null
    ok "Prerequisites installed: ${pkgs_to_install[*]}"
  fi
...
INITDB
  ...
  echo -e "  Backup:     sandcastle-admin backup"
