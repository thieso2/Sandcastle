...
cmd_destroy() {
  ...
  if [ -x "$DOCKER" ]; then
    ...
    if [ -f "$SANDCASTLE_HOME/docker-compose.yml" ]; then
      # Do NOT pass --volumes: the pgdata named volume holds the postgres database
      # and must survive uninstall so data is preserved across reinstalls.
  ...
  if $DOCKER volume inspect pgdata &>/dev/null 2>&1; then
    warn "Postgres data preserved in Docker volume 'pgdata' — run '$DOCKER volume rm pgdata' to delete it"
  fi
