...
# Apply cross-tenant isolation rules on the sandcastle-web bridge network.
# Blocks sandbox-to-sandbox traffic while allowing infra↔sandbox connections.
...
setup_network_isolation() {
  ...
  # Get the bridge interface for sandcastle-web
  local net_id
  net_id=$($DOCKER network inspect sandcastle-web --format '{{.Id}}' 2>/dev/null | head -c 12) || true
  if [ -z "$net_id" ]; then
    warn "sandcastle-web network not found — skipping network isolation setup"
    return
  fi
  local br="br-${net_id}"
  if ! ip link show "$br" &>/dev/null; then
    warn "sandcastle-web bridge ${br} not found — skipping network isolation setup"
    return
  fi
...
# Remove cross-tenant isolation chain
ExecStopPost=-/bin/bash -c 'iptables -D FORWARD -j SANDCASTLE-ISOLATION 2>/dev/null; iptables -F SANDCASTLE-ISOLATION 2>/dev/null; iptables -X SANDCASTLE-ISOLATION 2>/dev/null'
...
cmd_stop() {
    ...
    # Remove cross-tenant isolation chain
    iptables -D FORWARD -j SANDCASTLE-ISOLATION 2>/dev/null || true
